...
#define LPC313x_MCI_DATA_ERROR_FLAGS	(SDMMC_INT_DRTO | SDMMC_INT_DCRC | SDMMC_INT_HTO | SDMMC_INT_SBE | SDMMC_INT_EBE)
		...
		struct lpc313x_mci_slot *slot, struct mmc_data *data)
{
	 ...
	 */
	mci_writel(host, TMOUT, /*0xffffffff); */ (timeout << 8) | (70));
}
		...
		struct mmc_command *cmd, u32 cmd_flags)
{
	...
	mci_writel(host, CMDARG, cmd->arg); // write to CMDARG register
	mci_writel(host, CMD, cmd_flags | SDMMC_CMD_START); // write to CMD register

	/* wait until CIU accepts the command */
	while (--tmo && (mci_readl(host, CMD) & SDMMC_CMD_START))
		cpu_relax();
}
...
static int lpc313x_mci_submit_data_dma(struct lpc313x_mci *host, struct mmc_data *data)
{
	...
	for (i = 0, j = 0; i < sg_len; i++) {
		...
		while (length) {
			...
			if (data->flags & MMC_DATA_READ) {
				host->sg_cpu[j].setup.src_address = 0x18000100;
				...
				host->sg_cpu[j].setup.dest_address = 0x18000100;
	...
	mci_writel(host, CTRL, mci_readl(host, CTRL) | SDMMC_CTRL_DMA_ENABLE); // enable dma
...
#define mci_send_cmd(host, cmd, arg) {		\
		...
		while (mci_readl(host, CMD) & SDMMC_CMD_START);	\
}
...
static void lpc313x_mci_enable_sdio_irq(struct mmc_host *mmc, int enable)
{
	...
	if (enable) {
		reg = mci_readl(host, INTMASK) | SDMMC_INT_SDIO(0);
		mci_writel(host, INTMASK, reg);
	}
	else {
		reg = mci_readl(host, INTMASK) & ~SDMMC_INT_SDIO(0);
		mci_writel(host, INTMASK, reg);
...
static void lpc313x_mci_tasklet_func(unsigned long priv)
{
	...
	enum dw_mci_state	state = host->state;
...
inline static void lpc313x_mci_push_data(struct lpc313x_mci *host, void *buf, int cnt)
{
    ...
    cnt = cnt >> 2;
    while (cnt > 0) {
	    mci_writel(host, CDTHRCTL, *pData++);
	    cnt--;
    }
}
...
static void lpc313x_mci_read_data_pio(struct lpc313x_mci *host)
{
	...
	do {
		len = SDMMC_GET_FCNT(mci_readl(host, STATUS)) << 2;
		if(count == 0)
			old_len = len;
		...
		mci_writel(host, RINTSTS,SDMMC_INT_RXDR); // clear RXDR interrupt
...
static void lpc313x_mci_write_data_pio(struct lpc313x_mci *host)
{
	...
	do {
		...
		len = SDMMC_FIFO_SZ - (SDMMC_GET_FCNT(mci_readl(host, STATUS)) << 2);
		if (likely(offset + len <= sg->length)) {
		...
		status = mci_readl(host, MINTSTS);
		mci_writel(host, RINTSTS,SDMMC_INT_TXDR); // clear RXDR interrupt
		if (status & LPC313x_MCI_DATA_ERROR_FLAGS) {
			host->data_status = status;
			data->bytes_xfered += nbytes;
			smp_wmb();
...
static irqreturn_t lpc313x_mci_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		if(pending & LPC313x_MCI_CMD_ERROR_FLAGS) {
			mci_writel(host, RINTSTS,LPC313x_MCI_CMD_ERROR_FLAGS);  //  clear interrupt
		    host->cmd_status = status;
		    smp_wmb();
		    lpc313x_mci_set_pending(host, EVENT_CMD_COMPLETE);
		    tasklet_schedule(&host->tasklet);
		...
		if (pending & LPC313x_MCI_DATA_ERROR_FLAGS) { // if there is an error, let report DATA_ERROR
			mci_writel(host, RINTSTS,LPC313x_MCI_DATA_ERROR_FLAGS);  // clear interrupt
			host->data_status = status;
			smp_wmb();
			lpc313x_mci_set_pending(host, EVENT_DATA_ERROR);
			tasklet_schedule(&host->tasklet);
		...
		if(pending & SDMMC_INT_DATA_OVER) {
		    mci_writel(host, RINTSTS,SDMMC_INT_DATA_OVER);  // clear interrupt
		    if (!host->data_status)
			host->data_status = status;
		    smp_wmb();
		...
		if (pending & SDMMC_INT_RXDR) {
		    mci_writel(host, RINTSTS,SDMMC_INT_RXDR);  //  clear interrupt
		    if(host->sg)
			    lpc313x_mci_read_data_pio(host);
		}

		if (pending & SDMMC_INT_TXDR) {
		    mci_writel(host, RINTSTS,SDMMC_INT_TXDR);  //  clear interrupt
		    if(host->sg) {
		...
		if (pending & SDMMC_INT_CMD_DONE) {
		    mci_writel(host, RINTSTS,SDMMC_INT_CMD_DONE);  //  clear interrupt
		    lpc313x_mci_cmd_interrupt(host, status);
		}

		if (pending & SDMMC_INT_SDIO(0)) {
			mci_writel(host, RINTSTS, SDMMC_INT_SDIO(0));  //  clear interrupt
	...
	if (present != present_old) {
		...
		if (mrq) {
			if (mrq == host->mrq) {
			  	...
			  	while (mci_readl(host, CTRL) & (SDMMC_CTRL_RESET | SDMMC_CTRL_FIFO_RESET | SDMMC_CTRL_DMA_RESET));
...
static int lpc313x_mci_probe(struct platform_device *pdev)
{
	...
	clk_put(clk);
        ...
        __raw_writel(0, (void __iomem *)io_p2v(SYS_PHYS + 0x2c));
        __raw_writel(1, (void __iomem *)io_p2v(SYS_PHYS + 0x94));

        GPIO_DRV_IP(IOCONF_EBI_MCI, 0xF0000003);

        __raw_writel(0x1b, (void __iomem *)io_p2v(SYS_PHYS + 0x2c));
...
#ifdef BURST_DMA
	mci_writel(host, FIFOTH, ((0x1 << 28) | (0xB << 16) | (0xC << 0))); // RXMark = 11, TXMark = 12, DMA Size = 4
#else
  	mci_writel(host, FIFOTH, ((0x2 << 28) | (0x10 << 16) | (0x10 << 0))); // RXMark = 16, TXMark = 16, DMA Size = 8
#endif
	...
	mci_writel(host, RINTSTS, 0xFFFFFFFF);
	mci_writel(host, INTMASK,SDMMC_INT_CMD_DONE | SDMMC_INT_DATA_OVER | SDMMC_INT_TXDR | SDMMC_INT_RXDR | LPC313x_MCI_ERROR_FLAGS);
...
#else
#define lpc313x_mci_suspend NULL
#define lpc313x_mci_resume NULL
#endif /* CONFIG_PM */
